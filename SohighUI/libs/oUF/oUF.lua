	...
	local parent, ns = debugstack():match[[\AddOns\(.-)\]], {}
	...
	local print = function(...) print('|cff33ff99[oUF]:|r', ...) end
		...
		do
			...
			OnTargetUpdate = function(self, elapsed)
				if(not self.unit) then
					return
				elseif(timer >= .5) then
					--ChatFrame1:AddMessage('target update')
	...
	for k, v in pairs{
		...
		UpdateElement = function(self, name)
			...
			--ChatFrame1:AddMessage('Update single element: ' .. name)
	...
	local initObject = function(unit, style, styleFunc, ...)
		...
		for i = 1, num do
			...
			if(height) then
				...
				if(not combat) then object:Height(height) end
			end

			if(width) then
				object:SetAttribute('initial-width', width)
				if(not combat) then object:Width(width) end
			end
			...
			for element in next, elements do
				if(tostring(unit) == 'target' or tostring(unit) == 'player') then
					--ChatFrame1:AddMessage('unit: ' .. tostring(unit) .. ' - ' .. tostring(element) .. ' - ' .. tostring(next))
				end
