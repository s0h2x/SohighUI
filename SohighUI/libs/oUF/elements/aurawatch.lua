	...
	Here is an example of how to set oUF_AW up:
		...
		local createAuraWatch = function(self, unit)
			...
			for i, sid in pairs(spellIDs) do
				...
				icon:Width(24)
				icon:Height(24)
				icon:SetAnchor("BOTTOM", self, "BOTTOM", 0, 28 * i)
	...
	-- helper function backport from 3.0
	local UnitAura = function(unit, index, filter)
		-- if it's a specifik entry
		local indexIsInt = (n == math.floor(index))

		-- check for int now and return before the manual traversal.
		if indexIsInt then
			local intBased = UnitBuff(unit,index)
			if intBased == nil then
				intBased = UnitDebuff(unit,index)
			end
			return intBased
		end
		...
		-- Debuff time mother fuckers
		local j = 1
		local debuff = UnitDebuff(unit,j)
		while debuff and j < 41 do
			if select(1,debuff) == index then
				return debuff
			end
			j = j+1
			debuff = UnitDebuff(unit,j)
		end
	...
	local function setupIcons(self)
		...
		for _, icon in pairs(icons) do
			...
			if not icon.count and not (watch.hideCount or icon.hideCount) then
				...
				count:SetAnchor("BOTTOMRIGHT", icon, "BOTTOMRIGHT", -1, 0)
				icon.count = count
			end
