	...
	local onUpdate = function(self, elapsed)
		if(self.casting) then
			...
			if(self.SafeZone) then
				...
				self.SafeZone:Width(width * safeZonePercent)
			end
			...
			if(self.Spark) then
				self.Spark:SetAnchor("CENTER", self, "LEFT", (duration / self.max) * self:GetWidth(), 0)
			end
	...
	local Enable = function(object, unit)
		...
		if(castbar) then
			...
			if(object.unit == "player") then
				...
				CastingBarFrame.Show = E.hoop
				CastingBarFrame:Hide()
			elseif(object.unit == 'pet') then
				PetCastingBarFrame:UnregisterAllEvents()
				PetCastingBarFrame.Show = E.hoop
