	...
	local tagStrings = {
		...
		["difficulty"] = [[function(u)
			if UnitCanAttack("player", u) then
				local l = UnitLevel(u)
				return Hex(E.GetQuestDifficultyColor((l > 0) and l or 99))
			end
		end]],
		...
		if(not func) then
			...
			if(numTags == 1) then
				func = function(self)
					...
					--self:SetFont(A.font, 24, A.fontStyle)
					return self:SetFormattedText(format, args[1](parent.unit, realUnit) or '')
				end
